...
// Braze config (set env vars BRAZE_REST_ENDPOINT, BRAZE_REST_API_KEY)
...
{
    builder.Services.AddHttpClient<SegmentBackend.Services.BrazeClient>(client => { })
        .AddTypedClient((http, sp) => new SegmentBackend.Services.BrazeClient(http, brazeEndpoint!, brazeApiKey!));
}
