...
public class UrbanSiegeEngine
{
    ...
    public void Run()
    {
        Console.WriteLine($"=== {scenario.Name} Simulation ===");
        while (!IsFinished())
        {
            Step();
            t += dt;
    ...
    private void HandleDrones()
    {
        foreach (var d in scenario.Drones)
        {
            ...
            if (d.Type == DroneType.Hunter && dist < 3)
            {
                ...
                Console.WriteLine($"[{t:F1}s] Drone (Hunter) hit attacker at {target.Pos.Y:F1}m");
            }
            else if (d.Type == DroneType.Bomber && dist < 10 && rng.NextDouble() < 0.1)
            {
                ...
                Console.WriteLine($"[{t:F1}s] Drone (Bomber) explosion — {victims.Count} attackers KO");
                d.Cooldown = 300;
            }
        }
    }
        ...
        foreach (var def in scenario.Defenders)
        {
            foreach (var shooter in def.Soldiers.Where(s => !s.IsKO))
            {
                ...
                foreach (var t in targets)
                {
                    ...
                    {
                        t.IsKO = true;
                        Console.WriteLine($"[{t:F1}s] Defender {def.Name} killed attacker at {t.Pos.Y:F1}m");
                    }
                }
            }
        }
    ...
    private void AttackersFire()
    {
        foreach (var a in scenario.Attackers.Soldiers.Where(s => !s.IsKO && s.Pos.Y >= 70))
        {
            ...
            foreach (var d in defenders)
            {
                ...
                {
                    d.IsKO = true;
                    Console.WriteLine($"[{t:F1}s] Attacker hit defender {d.TeamName}");
                }
            }
        }
    }
    ...
    private void PrintOutcome()
    {
        ...
        Console.WriteLine($"Attackers alive: {aliveAttackers}");
        Console.WriteLine($"Defenders alive: {aliveDefenders}");

        if (aliveAttackers >= scenario.VictoryThreshold)
            Console.WriteLine(">> RESULT: ATTACKERS WIN");
        else
            Console.WriteLine(">> RESULT: DEFENDERS HOLD");
    }
}