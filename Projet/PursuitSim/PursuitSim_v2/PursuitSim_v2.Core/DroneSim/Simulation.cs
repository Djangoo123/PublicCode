...
public sealed class Simulation
{
    ...
    void LogHeader()
    {
        LogMessage?.Invoke("t(s)\tState\tAlive\tHeadY\tDroneState\tDronePos\tEvent");
    }

    void Log(string evt = "")
    {
        LogMessage?.Invoke($"{t,4:F0}\t{Team.State,-10}\t{Team.AliveCount}\t{Team.HeadY,6:F0}\t{Drone.State,-9}\t{Drone.Pos}\t{evt}");
